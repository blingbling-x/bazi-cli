...
const outputOption = { type: 'string', short: 'o' };
...
// ── Markdown formatters ──
function formatPillar(name, pillar) {
    const t = pillar.天干;
    const d = pillar.地支;
    const canggan = d.藏干;
    const lines = [];
    ...
    const parts = [];
    if (canggan.主气)
        parts.push(`主气${canggan.主气.天干}(${canggan.主气.十神})`);
    if (canggan.中气)
        parts.push(`中气${canggan.中气.天干}(${canggan.中气.十神})`);
    if (canggan.余气)
        parts.push(`余气${canggan.余气.天干}(${canggan.余气.十神})`);
...
function formatBaziMd(result) {
    ...
    if (rel && Object.keys(rel).length > 0) {
        ...
        lines.push('## 刑冲合会');
        lines.push('');
        for (const [pillar, ganZhi] of Object.entries(rel)) {
            for (const [ganOrZhi, relations] of Object.entries(ganZhi)) {
...
function formatSolarTimesMd(result, bazi) {
    const lines = [];
...
function formatCalendarMd(result) {
    const lines = [];
    ...
    if (result.农历节日)
        lines.push(`| 农历节日 | ${result.农历节日} |`);
    if (result.公历节日)
        lines.push(`| 公历节日 | ${result.公历节日} |`);
...
function output(result, format, mdFormatter) {
    if (format === 'json') {
...
async function main() {
    ...
    switch (subcommand) {
        ...
        case 'fan': {
            ...
            output(result, format, () => formatSolarTimesMd(result, values.bazi));
            break;
        }
        case 'calendar': {
            const { values } = parseArgs({
